...
class SlideForgeApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(body, text="Input slide").grid(row=0, column=0, sticky="w")
        ttk.Entry(body, textvariable=self.sdpc_var).grid(row=0, column=1, sticky="ew", padx=8)
        ttk.Button(body, text="Browse", command=self.pick_input).grid(row=0, column=2)
    ...
    def start_convert(self) -> None:
        ...
        thread = threading.Thread(target=self._run_process, args=(cmd, out_tif), daemon=True)
        thread.start()
    ...
    def _run_process(self, cmd: list[str], out_tif: Path) -> None:
        try:
            ...
            code = self.proc.wait()
            if code == 0:
                self._make_svs_link(out_tif)
                self._make_preview(out_tif)
                self.queue.put(("done", str(out_tif)))
            else:
